...
class ChatThreadListCreateView(APIView):
    ...
    def post(self, request):
        ...
        thread = ChatThread.objects.create(user=request.user, title=title or "")
...
class ChatThreadMessagesView(APIView):
    ...
    def _get_user_thread(self, request, thread_id):
        return ChatThread.objects.filter(id=thread_id, user=request.user).first()

    def get(self, request, thread_id):
        thread = self._get_user_thread(request, thread_id)
        if thread is None:
...
class ChatMessageFavoriteToggleView(APIView):
    ...
    def patch(self, request, message_id):
        message = ChatMessage.objects.select_related("thread").filter(id=message_id, thread__user=request.user).first()
        if message is None:
...
class FavoriteMessagesListView(APIView):
    ...
    def get(self, request):
        favorites = ChatMessage.objects.filter(thread__user=request.user, is_favorite=True).select_related("thread")
        return Response(
            {
